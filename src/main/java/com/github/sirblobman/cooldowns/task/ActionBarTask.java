...
public final class ActionBarTask extends CooldownTask {
    ...
    private void checkActionBar(Player player) {
        ...
        Set<CooldownSettings> activeCooldowns = cooldownData.getActiveCooldowns();
        if(activeCooldowns.isEmpty()) {
            printDebug("Player does not have any active cooldowns.");
            return;
        }

        List<CooldownSettings> cooldownSettingsList = activeCooldowns.parallelStream()
                .filter(settings -> settings.getActionBarSettings().isEnabled())
                .sorted(Comparator.comparing(CooldownSettings::getActionBarSettings).reversed())
        ...
        CooldownSettings cooldownSettings = cooldownSettingsList.get(0);
